...
// Nerve 用 data-theme 属性切换主题（不是 .dark class）
function isDark() {
...
function drawScene(s: Scene) {
  ...
  // 生成中（无图）：canvas 保持透明底，格子和沟槽半透明，
  // 让外层 div 的 backdrop-filter 毛玻璃透出来；图片就绪后铺实底
  const glassMode = !s.image;
  if (glassMode) {
  ...
  const cellAlpha = glassMode ? 0.55 : 1;
  ...
  const paint = (p: Patch) => {
    ...
    ctx.fillStyle = `rgba(${shade(grey, p.r)},${shade(grey, p.g)},${shade(grey, p.b)},${cellAlpha})`;
