 ...
 *
 * Dependencies: `better-sqlite3` (Electron-compatible) + `sqlite-vec`.
 *
 * Design:
 * - All operations are synchronous (DatabaseType API).
...
// better-sqlite3 replaces node:sqlite for Electron compatibility
...
export class VectorStore implements IMemoryStore {
  ...
  private stmtUpsertMeta!: StatementType;
  private stmtDeleteVec?: StatementType;   // optional — only set when vecTablesReady
  private stmtInsertVec?: StatementType;   // optional — only set when vecTablesReady
  private stmtDeleteMeta!: StatementType;
  private stmtGetMeta!: StatementType;
  private stmtSearchVec?: StatementType;   // optional — only set when vecTablesReady
  private stmtQueryBySessionId!: StatementType;
  private stmtQueryBySessionIdSince!: StatementType;
  private stmtQueryBySessionKey!: StatementType;
  private stmtQueryBySessionKeySince!: StatementType;
  private stmtQueryAll!: StatementType;
  private stmtQueryAllSince!: StatementType;

  // Prepared statements — L0 (initialized in init())
  private stmtL0UpsertMeta!: StatementType;
  private stmtL0DeleteVec?: StatementType;   // optional — only set when vecTablesReady
  private stmtL0InsertVec?: StatementType;   // optional — only set when vecTablesReady
  private stmtL0DeleteMeta!: StatementType;
  private stmtL0GetMeta!: StatementType;
  private stmtL0SearchVec?: StatementType;   // optional — only set when vecTablesReady
  /** L0 query for L1 runner: all messages for a session key */
  private stmtL0QueryAll!: StatementType;
  /** L0 query for L1 runner: messages after a timestamp cursor */
  private stmtL0QueryAfter!: StatementType;
  /** L1 cursor-based pagination for migration (by PK) */
  private stmtL1QueryMigrationCursor!: StatementType;
  /** L0 cursor-based pagination for migration (by PK) */
  private stmtL0QueryMigrationCursor!: StatementType;
  ...
  private stmtL1FtsInsert!: StatementType;
  private stmtL1FtsDelete!: StatementType;
  private stmtL1FtsSearch!: StatementType;

  // Prepared statements — FTS5 L0 (initialized in init())
  private stmtL0FtsInsert!: StatementType;
  private stmtL0FtsDelete!: StatementType;
  private stmtL0FtsSearch!: StatementType;
  ...
  constructor(dbPath: string, dimensions: number, logger?: Logger) {
    ...
    // Open database with better-sqlite3 (Electron-compatible)
  ...
  init(providerInfo?: EmbeddingProviderInfo): VectorStoreInitResult {
    // Load sqlite-vec extension (better-sqlite3 compatible)
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
