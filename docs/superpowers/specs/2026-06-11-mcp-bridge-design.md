...
├── tunnel.ts        # TunnelManager — cloudflared 进程管理
...
export class McpBridgeServer {
  ...
  getHealth(): { status: string; toolCount: number; port: number; tunnelUrl: string | null } {
    return { status: 'ok', toolCount: Object.keys(this.tools).length, port: this.config.port, tunnelUrl: this.tunnel.getUrl() }
  }
...
Nerve 不内嵌 cloudflared 二进制，通过 `child_process.spawn` 管理用户自行安装的 `cloudflared` 进程。使用 Cloudflare 的免费 Quick Tunnel（`--url` 模式），无需账号，自动分配随机子域名。

新增文件 `src/main/mcp-bridge/tunnel.ts`：
...
在 `McpBridgeServer` 中组合 `TunnelManager`：
...
export class McpBridgeServer {
  ...
  async start(): Promise<void> {
    ...
    // 如果 cloudflare enabled，自动拉起 tunnel
...
`~/.nerve/mcp-bridge.json` 增加 `cloudflare` 字段：
...
用户需自行安装 cloudflared：`npm i -g cloudflared`。启动失败时通过 IPC 返回清晰错误信息。

### 安全

- Quick Tunnel URL 不可预测（随机子域名），天然防扫描
- Bearer Token 认证在 tunnel 之上仍然生效
- tunnel 进程随 MCPBridgeServer.stop() 一起终止
